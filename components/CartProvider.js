...
export default function CartProvider({ children }) {
    ...
    const [ProductOverviewId, setProductOverviewId] = useState([])
    ...
    const addToCart = async (item) => {
        ...
        const existingItemIndex = cartItems.findIndex((cartItem) => cartItem.product._id == item._id)
        ...
        // Update productIds array with new product ID
    ...
    const removeItem = async (itemId) => {
        try {
            const updatedCartItems = cartItems.filter((cartItem) => cartItem.product._id !== itemId);
    ...
    const increaseQuantity = async (itemId) => {
        ...
        const updatedItem = updatedCartItems.find((item) => item.product._id == itemId);
    ...
    const decreaseQuantity = async (itemId) => {
        ...
        const data = {
            userId,
            productId: itemId,
            quantity: updatedCartItems.find((item) => item.product._id === itemId).quantity,
        };
    ...
    const contextValue = useMemo(() => ({
        ...
        orderId,
        setOrderId,
    }), [addToCart, removeItem, increaseQuantity, decreaseQuantity, setProductOverviewId, selectedAddressId, setSelectedAddressId, cartId, razorpayOrder, setRazorpayOrder, productIds, fetchCartItems, orderId, setOrderId]);
