...
export default function ShippinginfoPage() {
  ...
  return (
    <div
      className="container mx-auto py-8 flex flex-col gap-2  overflow-auto "
      style={{
        ...
        <>
          <div className="flex md:justify-between justify-start md:items-center w-full flex-col md:flex-row gap-3 h-full ">
            ...
            <div>
              <Link
                href={"/editaddress"}
                className=" border px-4 py-2 border-black   rounded hover:bg-black hover:text-white "
              >
                Add address
              </Link>
            </div>
          ...
          <h1 className="font-medium text-red-600 capitalize">
            default address
          </h1>
          {defaultAddress && (
            <label
              className={`border p-2 flex items-center ${
                selectedAddressId === defaultAddress._id
                  ? "border-indigo-500"
                  : "border-gray-300"
              } rounded-md bg-gray-200 shadow-md`}
            ...
            >
              ...
              >
                ...
                <div className="flex gap-2 ">
                  <Link
                    href={"/editaddress/" + defaultAddress._id}
                    className="w-32 text-center border px-4 py-2 border-black text-red-black rounded hover:bg-black hover:text-white"
                  >
                    Edit
                  </Link>
                  <button
                    onClick={() => handleRemoveAddress(defaultAddress._id)}
                    className="w-32 text-center border px-4 py-2 border-red-500 text-red-700 rounded hover:bg-red-600 hover:text-white"
                  >
                    Remove
                  </button>
                </div>
            ...
            <label
              ...
              className={`border p-2 flex items-center ${
                selectedAddressId === item._id
                  ? "border-indigo-500"
                  : "border-gray-300"
              } rounded-md bg-gray-100 shadow-md`}
              htmlFor={item._id}
            >
              <div
                key={item._id}
